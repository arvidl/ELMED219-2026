...
% TITTELINFO
% =====================================================================
\title[X: XAI]{Forklarbar AI (XAI)}
\subtitle{Momentliste X01--X08}
...
\begin{frame}{X01: Forklare hvorfor forklarbarhet er viktig i medisinsk AI}
    \textbf{Utfordringen med ``black box'' AI:}
    \begin{itemize}
    ...
    \begin{alertblock}{Medisinsk praksis}
        En lege som ikke kan forklare hvorfor de anbefaler en behandling vil miste pasientens tillit. Det samme gjelder AI-systemer.
    \end{alertblock}
\end{frame}
...
\begin{frame}{X02: Skille mellom global og lokal forklarbarhet}
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                \item LIME
                \item SHAP (individuelle verdier)
                \item Grad-CAM (for bilder)
            \end{itemize}
        \end{column}
    \end{columns}
...
\begin{frame}{X04: Beskrive SHAP (SHapley Additive exPlanations)}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textbf{Styrker:}
            \begin{itemize}
                \item Teoretisk solid (Shapley-aksiomer)
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                \item \textbf{Waterfall plot:} Én prediksjon
                \item \textbf{Summary plot:} Global oversikt
                \item \textbf{Force plot:} Interaktiv
            \end{itemize}
        \end{column}
    \end{columns}
...
\begin{frame}{X05: Beskrive LIME (Local Interpretable Model-agnostic Explanations)}
    \textbf{LIME: Lokal tilnærming med enkel modell}
    
    \vspace{0.3cm}
    \textbf{Hovedidé:}
    \begin{enumerate}
        \item Velg én prediksjon å forklare
        \item Generer \textbf{perturberte} (forstyrrede) versjoner av input
...
\begin{frame}{X06: Forklare Grad-CAM for CNN-visualisering}
    ...
    \begin{enumerate}
        ...
        \item Kombiner til et \textbf{heatmap} som overlays på originalbildet
    \end{enumerate}
...
\begin{frame}{X07: Diskutere begrensninger ved XAI-metoder}
    \textbf{XAI er ikke perfekt -- viktige begrensninger:}
...
\begin{frame}{X08: Kjenne til attention-visualisering i LLM}
    \textbf{Attention i LLM: Innebygd ``forklaring''?}
    
    \begin{itemize}
        \item Transformer-modeller bruker \textbf{self-attention}
