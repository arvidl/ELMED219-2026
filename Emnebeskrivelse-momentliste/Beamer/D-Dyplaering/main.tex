...
% TITTELINFO
% =====================================================================
\title[D: Dyplæring]{Nevrale nettverk og Dyplæring}
\subtitle{Momentliste D01--D13}
...
\begin{frame}{D03: Forklare hva en aktiveringsfunksjon er (ReLU, sigmoid)}
    \textbf{Hvorfor aktiveringsfunksjoner?}
    \begin{itemize}
        \item Uten dem: Hele nettverket = lineær transformasjon
        \item Introduserer \textbf{ikke-linearitet} -- kan lære komplekse mønstre
    \end{itemize}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                ...
                \item Rask å beregne
                \item Unngår vanishing gradients
            \end{itemize}
        \end{column}
    \end{columns}
    
    \vspace{0.3cm}
    \begin{block}{Softmax (for multi-klasse output)}
        $\text{softmax}(z_i) = \frac{e^{z_i}}{\sum_j e^{z_j}}$ -- gir sannsynlighetsfordeling over klasser
    \end{block}
\end{frame}
...
\begin{frame}{D04: Forstå konseptet forward propagation}
    ...
    \begin{enumerate}
        \item \textbf{Input:} Data $\mathbf{x}$ legges inn i inputlaget
...
\begin{frame}{D05: Forklare backpropagation på et konseptuelt nivå}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Kjerederegelen:} Deriver feil m.h.p. hver vekt i nettverket
    ...
    \begin{block}{Matematisk (forenklet)}
        ...
        \small (Kjerederegelen propagerer gradienter bakover gjennom nettverket)
    \end{block}
\end{frame}
...
\begin{frame}{D06: Forstå gradient descent og læringsrate}
    ...
    \vspace{0.3cm}
    \begin{block}{Epoch}
        En \textbf{epoch} = én gjennomgang av hele treningsdatasettet
    \end{block}
\end{frame}
...
\begin{frame}{D07: Kjenne til loss functions (cross-entropy, MSE)}
    ...
    \vspace{0.3cm}
    \begin{block}{Velge riktig loss}
        \begin{tabular}{ll}
            Regresjon & $\rightarrow$ MSE eller MAE \\
            Binær klassifisering & $\rightarrow$ Binary Cross-Entropy \\
            Multi-klasse klassifisering & $\rightarrow$ Categorical Cross-Entropy
        \end{tabular}
    \end{block}
\end{frame}
...
\begin{frame}{D10: Beskrive pooling-lag og deres funksjon}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                \item \textbf{Reduserer beregning} -- færre parametre i neste lag
                \item \textbf{Translasjonsinvarians} -- litt skift i bildet påvirker ikke output
                \item \textbf{Unngår overfitting} -- abstraherer bort detaljer
            \end{itemize}
        \end{column}
        \begin{column}{0.48\textwidth}
    ...
    \vspace{0.3cm}
    \begin{block}{Andre pooling-typer}
        \begin{itemize}
            \item \textbf{Average pooling:} Gjennomsnitt i stedet for max
            \item \textbf{Global average pooling:} Hele feature map $\rightarrow$ én verdi
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{D12: Forstå konseptet transfer learning}
    ...
    \vspace{0.3cm}
    \begin{alertblock}{Hvorfor fungerer det?}
        Tidlige lag lærer \textbf{generelle features} (kanter, teksturer) som er nyttige for mange oppgaver. Kun de siste lagene er oppgavespesifikke.
    \end{alertblock}
    
    \begin{block}{Eksempel i medisin}
        Tren på millioner av naturlige bilder $\rightarrow$ Fintun på tusenvis av røntgenbilder $\rightarrow$ Bedre resultat enn å trene fra scratch!
    \end{block}
\end{frame}
